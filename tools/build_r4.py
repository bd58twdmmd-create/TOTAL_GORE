import sys,zipfile,re,hashlib,collections
...
for m in blocks:
    ...
    if t.lower()=='sprite': b=re.sub(r'(?im)^\s*Offset\s+([-+0-9.eE]+)\s*,\s*([-+0-9.eE]+)\s*$',lambda q:f'    Offset {int(round(float(q.group(1))))}, {int(round(float(q.group(2))))}',b)
...
sha=hashlib.sha256(out.read_bytes()).hexdigest(); rep=out.with_name('GREZZODUE2_MOBILE_1TO1_FINAL_R4_VALIDATION.txt')
